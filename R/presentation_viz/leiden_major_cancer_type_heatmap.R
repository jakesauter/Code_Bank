...
leiden_communities <- 
  arrow::read_feather('data/3_iteration_leiden_clustering_results.feather') %>% 
  unlist() %>% 
  as.integer()

seurat_object@meta.data$leiden_clusters <- 
  leiden_communities
...
numeric_cell_count_per_cluster <- 
  metadata %>% 
  mutate(metacluster = as.integer(metacluster)) %>% 
  count(metacluster) %>% 
  .$n
...
metadata %>% 
  ...
  tidyr::complete(metacluster, cancer_type_detailed) %>% 
  rowwise() %>% 
  mutate(n = n / numeric_cell_count_per_cluster[as.integer(metacluster)]) %>%
  ungroup() %>% 
  ggplot() + 
  geom_tile(aes(x = cancer_type_detailed,
                y = metacluster,
