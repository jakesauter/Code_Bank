...
for (assay in c('M1', 'M2')) {
  ...
  for (i in seq_along(flow_directories)) {
    ...
    fcs_path <- 
      file.path(flow_directories[[i]], 
                paste0('processed_', assay, '_subsampled_100k.fcs'))
  ...
  # Save Seurat object
  saveRDS(seurat_obj, 
          file.path(SAVE_DIR, paste0(assay, '_SPADE_centralized_Seurat_object.Rds')))
  ...
  arrow::write_feather(seurat_expression_data,
                       file.path(SAVE_DIR, paste0(assay, '_SPADE_seurat_expression_data.feather')))
}
