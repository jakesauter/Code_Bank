...
### Input data
flow_dir_save_loc <- 
  "/Users/sauterj1/Documents/Woodlist/Full_Correct_QC_Flow_Folders/"

path_to_ids_df <- 
  "/Users/sauterj1/Documents/Patient_Folder_Analysis/data/flow_df_ids_052621.csv"
...
cypher_df <- 
  read.csv(path_to_ids_df)

anon_flow_directories <-
  basename(flow_directories) %>% 
  str_extract('F[0-9]+-[0-9]+') %>% 
  sapply(function(flow_id) 
    cypher_df[cypher_df$Accession.Number == flow_id, ]$PRPT_PATH_RPT_ID
  ) %>% 
  unlist() %>% unname() %>% 
  fs::path(anon_flow_dir_loc, .)
...
for (i in seq_along(flow_directories)) {
  ...
  cat('\n\nFlow Directory: ', i, " out of: ", length(flow_directories), '\n')
